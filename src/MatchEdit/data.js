...
const saveMatch = (values, setSubmitting, sleep, success, failure) => {
  return new Promise((resolve, failed) => {
    const encodedQuery = encodeURIComponent(values.id || '');
    API.post(`match/${encodedQuery}`, values)
      .then(res => {
        return setTimeout(() => success(res.data, setSubmitting), sleep);
      })
      .catch(error => {
        let msg;
        if (!error.response || error.code === 'ECONNABORTED') {
